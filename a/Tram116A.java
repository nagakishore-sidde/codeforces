...
public class Tram116A {
	public static void main(String[] args) {
		...
		for (int i=0; i<count; i++) {
			...
			totalCount += enterCount - exitCount;
			if (totalCount > res)
				res = totalCount;
		}
 ...
 * n in the order of tram's movement. At the i-th stop ai passengers exit the
 ...
 * — the number of passengers that exits the tram at the i-th stop, and the
 * number of passengers that enter the tram at the i-th stop. The stops are
 * given from the first to the last stop in the order of tram's movement.
 * 
 * The number of people who exit at a given stop does not exceed the total
 * number of people in the tram immediately before it arrives at the stop. More
 * formally, . This particularly means that a1 = 0. At the last stop, all the
 * passengers exit the tram and it becomes empty. More formally, . No passenger
 ...
 * is allowed). Sample test(s) Input
 * 
 * 4 0 3 2 5 4 2 4 0
 * 
 * Output
 ...
 * is 0. Then, 3 passengers enter the tram, and the number of passengers inside
 * the tram becomes 3. At the second stop, 2 passengers exit the tram (1
 * passenger remains inside). Then, 5 passengers enter the tram. There are 6
 * passengers inside the tram now. At the third stop, 4 passengers exit the tram
 * (2 passengers remain inside). Then, 2 passengers enter the tram. There are 4
 ...
 * tram now, which is in line with the constraints.
 * 
